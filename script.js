...
// 粒子历史位置（用于拖尾效果）
let particleHistory = [];
const SHAPE_SEQUENCE = ['heart', 'flower', 'saturn', 'sphere', 'star', 'ring', 'cube', 'spiral', 'snowflake', 'torus'];
...
function getHandSize(hand) {
    ...
    return Math.max(getDistance(keypoints[0], keypoints[9]), 1);
}
...
function selectShape(shape) {
    if (!shape || currentShape === shape) return;
    currentShape = shape;
...
function switchShape(direction) {
    const currentIndex = Math.max(0, SHAPE_SEQUENCE.indexOf(currentShape));
    const nextIndex = (currentIndex + direction + SHAPE_SEQUENCE.length) % SHAPE_SEQUENCE.length;
    selectShape(SHAPE_SEQUENCE[nextIndex]);
}

function isHeartGesture(hands) {
    if (hands.length < 2) return false;
    const first = hands[0].keypoints;
    const second = hands[1].keypoints;
    if (!first || !second || first.length < 21 || second.length < 21) return false;

    const palmDistance = getDistance(getPalmCenter(hands[0]), getPalmCenter(hands[1]));
    if (palmDistance < 1) return false;

    const thumbDistance = getDistance(first[4], second[4]);
    const indexDistance = getDistance(first[8], second[8]);
    const thumbIndexCrossA = getDistance(first[4], second[8]);
    const thumbIndexCrossB = getDistance(second[4], first[8]);

    return (
        thumbDistance < palmDistance * 0.42 &&
        indexDistance < palmDistance * 0.55 &&
        thumbIndexCrossA < palmDistance * 0.8 &&
        thumbIndexCrossB < palmDistance * 0.8
    );
}

function updateParticleSizeFromPinch(hand) {
    const keypoints = hand.keypoints;
    if (!keypoints || keypoints.length < 21 || !particleSystem) return false;

    const pinchDistance = getDistance(keypoints[4], keypoints[8]);
    const handSize = getHandSize(hand);
    const normalizedPinch = pinchDistance / handSize;
    const isPinching = normalizedPinch < 1.25;
    ...
    if (previousPinchDistance > 0) {
        const delta = (pinchDistance - previousPinchDistance) / handSize;
        if (Math.abs(delta) > 0.015) {
            particleSize = clampValue(particleSize + delta * 0.9, 0.05, 3);
            particleSystem.material.size = particleSize;
            particleSystem.material.needsUpdate = true;

            const sizeSlider = document.getElementById('size-slider');
            const sizeValue = document.getElementById('size-value');
            if (sizeSlider) sizeSlider.value = particleSize.toFixed(2);
            if (sizeValue) sizeValue.textContent = particleSize.toFixed(2);
        }
    }
...
function handleHandGesture(hands) {
    ...
    if (particleSystem && hands.length >= 2 && isHeartGesture(hands) && now - lastHeartTime > 1200) {
        selectShape('heart');
        lastHeartTime = now;
        displayGesture = 'heart';
    }

    if (particleSystem && palmCenter && previousPalmCenter) {
        const deltaX = palmCenter.x - previousPalmCenter.x;
        const deltaY = palmCenter.y - previousPalmCenter.y;

        particleSystem.rotation.y += deltaX * 0.004;
        particleSystem.rotation.x += deltaY * 0.0025;

        if (Math.abs(deltaX) > 70 && Math.abs(deltaX) > Math.abs(deltaY) * 1.6 && now - lastSwipeTime > 900) {
            switchShape(deltaX > 0 ? 1 : -1);
            lastSwipeTime = now;
            displayGesture = deltaX > 0 ? 'swipe-right' : 'swipe-left';
        } else if (displayGesture !== 'heart') {
            displayGesture = 'rotate';
        }
    }

    previousPalmCenter = palmCenter;

    if (particleSystem && hands.length >= 2) {
        const firstPalm = getPalmCenter(hands[0]);
        const secondPalm = getPalmCenter(hands[1]);
        const twoHandDistance = getDistance(firstPalm, secondPalm);

        if (previousTwoHandDistance > 0 && twoHandDistance > 0) {
            const ratio = twoHandDistance / previousTwoHandDistance;
            if (Math.abs(ratio - 1) > 0.02) {
                targetScale = clampValue(targetScale * ratio, 0.2, 5.0);
                if (displayGesture !== 'heart' && !String(displayGesture).startsWith('swipe')) {
                    displayGesture = ratio > 1 ? 'two-hands-open' : 'two-hands-close';
                }
            }
        }
    ...
    if (particleSystem && updateParticleSizeFromPinch(hand)) {
        if (displayGesture !== 'heart' && !String(displayGesture).startsWith('swipe')) {
    ...
    if (particleSystem) {
        if (gesture === 'fist') {
            const scaleFactor = Math.max(0.5, 1 - particleSpeed * 0.1);
            targetScale = Math.max(0.2, targetScale * scaleFactor);
        } else if (gesture === 'open') {
            const scaleFactor = Math.min(1.5, 1 + particleSpeed * 0.1);
    ...
    updateGestureIndicator(displayGesture);
}
...
function updateGestureIndicator(gesture) {
    ...
    if (gesture === 'fist') {
        ...
        gestureIcon.className = 'fas fa-sync-alt gesture-icon';
        gestureText.textContent = '手掌移动 - 旋转';
    } else if (gesture === 'swipe-left') {
        gestureIcon.className = 'fas fa-arrow-left gesture-icon';
        gestureText.textContent = '左挥 - 上个形状';
    } else if (gesture === 'swipe-right') {
        gestureIcon.className = 'fas fa-arrow-right gesture-icon';
        gestureText.textContent = '右挥 - 下个形状';
    } else if (gesture === 'pinch') {
        gestureIcon.className = 'fas fa-compress-alt gesture-icon';
        gestureText.textContent = '捏合 - 粒子大小';
    } else if (gesture === 'two-hands-open') {
        gestureIcon.className = 'fas fa-expand-arrows-alt gesture-icon';
        gestureText.textContent = '双手拉开 - 整体放大';
    } else if (gesture === 'two-hands-close') {
        gestureIcon.className = 'fas fa-compress-arrows-alt gesture-icon';
        gestureText.textContent = '双手靠近 - 整体缩小';
    } else if (gesture === 'heart') {
        gestureIcon.className = 'fas fa-heart gesture-icon';
        gestureText.textContent = '比心 - 爱心特效';
    ...
    canvas.addEventListener('touchmove', (e) => {
        ...
        if (e.touches.length === 1 && isRotating) {
            ...
            if (previousTouchDistance > 0) {
                targetScale *= distance / previousTouchDistance;
                clampTargetScale();
            }
...
function initUIEvents() {
    if (window.matchMedia('(max-width: 768px)').matches) {
        ...
        const toggleIcon = document.querySelector('#toggle-panel i');
